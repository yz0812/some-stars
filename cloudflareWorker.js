    ...
    return repos.map((repo) => {
      ...
      return `
        <div class="repo-card max-w-sm rounded-2xl overflow-hidden shadow-lg m-4 bg-white transition-transform transform hover:scale-105">
          ...
          <div class="px-6 pt-4 pb-2">
            <span class="inline-block bg-blue-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2" style="position: absolute; bottom: 15;">Stars: ${repo.stargazers_count}</span>
          </div>
        </div>`;
    }).join('');
  };
